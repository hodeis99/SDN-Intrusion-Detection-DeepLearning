...
from tensorflow.keras.layers import Dense, LSTM, Conv2D, MaxPooling2D, Reshape, Flatten, Dropout
...
def create_hybrid_model(input_shape, l2_lambda=0.1, dropout_rate1=0.25, dropout_rate2=0.5):
    model = Sequential()
    ...
    # Prepare for LSTM - reshape CNN output to time series data
    model.add(Reshape((-1, 64)))  # Convert to (timesteps, features)
...
plt.title('Confusion Matrix with ADASYN')
...
plt.title('Receiver Operating Characteristic (ROC) Curve with ADASYN')
...
plt.title('Model Accuracy with ADASYN')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
...
plt.title('Model Loss with ADASYN')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
...
# Save the model
model.save('/content/drive/MyDrive/Fereshte/hybrid_cnn_lstm_model_adasyn.h5')
print("Model with ADASYN saved successfully!")

# Compare with individual models (CNN only and LSTM only) with ADASYN
...
# Create and train CNN model with ADASYN
cnn_model = create_cnn_model(input_shape)
cnn_model.compile(optimizer=Adam(learning_rate=0.001),
                 loss='categorical_crossentropy',
                 metrics=['accuracy', 'precision', 'recall'])

cnn_history = cnn_model.fit(X_train, y_train_categorical,
                           batch_size=64,
                           epochs=50,
                           validation_split=0.2,
                           verbose=0)

cnn_test_loss, cnn_test_accuracy, cnn_test_precision, cnn_test_recall = cnn_model.evaluate(
    X_test, y_test_categorical, verbose=0
)

# Create and train LSTM model with ADASYN
lstm_model = create_lstm_model(input_shape)
lstm_model.compile(optimizer=Adam(learning_rate=0.001),
                  loss='categorical_crossentropy',
                  metrics=['accuracy', 'precision', 'recall'])

lstm_history = lstm_model.fit(X_train, y_train_categorical,
                             batch_size=64,
                             epochs=50,
                             validation_split=0.2,
                             verbose=0)

lstm_test_loss, lstm_test_accuracy, lstm_test_precision, lstm_test_recall = lstm_model.evaluate(
    X_test, y_test_categorical, verbose=0
)

# Print comparison results
print("Model Comparison Results with ADASYN:")
print("=" * 60)
print(f"Hybrid CNN-LSTM - Accuracy: {test_accuracy:.4f}, Precision: {test_precision:.4f}, Recall: {test_recall:.4f}, F1: {test_f1:.4f}")
print(f"CNN Only        - Accuracy: {cnn_test_accuracy:.4f}, Precision: {cnn_test_precision:.4f}, Recall: {cnn_test_recall:.4f}")
print(f"LSTM Only       - Accuracy: {lstm_test_accuracy:.4f}, Precision: {lstm_test_precision:.4f}, Recall: {lstm_test_recall:.4f}")

# Create comparison plot
models = ['CNN Only', 'LSTM Only', 'Hybrid CNN-LSTM']
accuracies = [cnn_test_accuracy, lstm_test_accuracy, test_accuracy]

plt.figure(figsize=(10, 6))
bars = plt.bar(models, accuracies, color=['lightblue', 'lightgreen', 'salmon'])
plt.title('Model Comparison - Accuracy (with ADASYN)')
plt.ylabel('Accuracy')
plt.ylim(0, 1)
...
# Clear keras session and close any open resources
from tensorflow.keras import backend as K
import gc

# Clear session and free up resources
K.clear_session()
gc.collect()
